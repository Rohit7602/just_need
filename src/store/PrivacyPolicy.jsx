...
function PolicyProvider({ children }) {
    ...
    const fetchPolicy = async () => {
      const { data, error } = await supabase.from("policy").select("*").eq("id",2).single();
      if(!error){
        return data
      }
  ...
  return <PolicyContext.Provider
  value={{AddPolicy,fetchPolicy}}
  >{children}</PolicyContext.Provider>;
}
